...
const loaderImpl = (key: string): Promise<{ value: string; size: number; key: string }> => {
	return new Promise((resolve, reject) => {
		...
		setTimeout(() => {
			resolve({ value: target.value, size: target.size, key: "key_" + key });
		}, DEFAULT_LOAD_TIME);
	});
};
...
describe("CachedLoader", () => {
	it("リソース・キャッシュを取得できる", async () => {
		...
		expect(resource.key).toBe("key_id1");
		...
		expect(resource2.key).toBe("key_id1");
	});
		...
		const loaderImpl = (key: string): Promise<{ value: string; size: number; key: string }> => {
			...
			return new Promise((resolve, reject) => {
				...
				if (count < 2) {
					...
					resolve({ value: "success", size: 10, key });
				}
			});
		};
		const loader = new CachedLoader<string, string>(loaderImpl, { limitSize: 1000 });
	...
	it("キャッシュの上限を超えた場合、最も利用日時が古いリソースからキャッシュから削除される", async () => {
		...
		expect(resource1.key).toBe("key_id1");
		...
		expect(resource2.key).toBe("key_id2");
