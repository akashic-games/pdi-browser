...
export class HTMLAudioAsset extends AudioAsset {
	...
	_cachedLoader: CachedLoader<string, { audio: HTMLAudioElement; url: string }> | null = null;

	static async _loadImpl(url: string): Promise<{ value: { audio: HTMLAudioElement; url: string }; size: number }> {
		try {
	...
	_load(loader: pdi.AssetLoadHandler): void {
		...
		const load = this._cachedLoader ? this._cachedLoader.load.bind(this._cachedLoader) : HTMLAudioAsset._loadImpl;
